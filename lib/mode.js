...
export function renderList(available, n) {
	const lines = [
	...
	lines.push(
		`KDF: scrypt N=${n} (2^${Math.log2(n)}) r=1 p=1 — probed for THIS machine; ` +
			'lock and open a vault on machines with the same effective N',
	)
	return lines.join('\n').replace(/\n$/, '')
}
